...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    login({ name: form.email.split('@')[0], email: form.email, uid: 'user-' + Date.now() })
  ...
  return (
    <div className="min-h-screen bg-[#050510] bg-grid flex items-center justify-center px-4">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md relative">
        ...
        <div className="glass-strong rounded-3xl p-8 border border-white/10">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button type="submit" variant="primary" size="lg" loading={loading} className="w-full mt-2" icon={ArrowRight}>Sign In</Button>
          </form>
          <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl">
            <p className="text-xs text-blue-300 text-center">Demo mode: enter any email + password to sign in</p>
          </div>
